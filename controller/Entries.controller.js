...
export const createEntries = async (req, res) => {
    try {
        ...
        const { __v: bs, _id: dd, userId: vv, ...salesLeft } = savedSalesTarget._doc;
...
export const updateEntries = async (req, res) => {
    try {
        ...
        const { __v: bs, _id: dd, userId: vv, ...salesLeft } = updatedSalesTarget._doc;
...
export const updateDailyEntries = async (req, res) => {
    try {
        ...
        if (name === "prospectingApproach") {
            valueToBeUpdated = { "prospectingapproach.achieved": value };
        } else if (name === "appointmentsKept") {
            valueToBeUpdated = { "appointmentskept.achieved": value };
        } else if (name === "salesSubmitted") {
            valueToBeUpdated = { "salessubmitted.achieved": value };
        }

        // Find and update the success formula by user ID and date
        const updatedSuccessFormula = await SuccessFormula.findOneAndUpdate(
            { userId, date },
        ...
        res.status(200).json({
            success: true,
            updatedSuccessEntries: successLeft,
        });
    } catch (error) {
        console.error("Error updating entries:", error);
        res.status(500).json({ success: false, message: "Internal server error" });
